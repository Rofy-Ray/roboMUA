...
# rec = pd.read_json('./content/res.json')
with open('./content/res.json', 'r') as f:
    rec = json.load(f)
   ...
   # ax2.set_title('Class Probability')
    ...
    for key,value in rec.items():
        for k,v in value.items():
            if k == 'SkinTone' and v == skintype:
                fin_res.append(value)
    rec1 = json.dumps(fin_res)
    print(rec1)
    # rec1 = [rec['']['SkinTone'] == skintype]
...
def result():
    ...
    return render_template("results.html", table_results = res)
